...
function afficherContenu(employee, reviews, compensations) {
    ...
    `).join('') : `<p class="empty-hint">Aucun entretien enregistré pour le moment.</p>`;
    ...
    compContainer.innerHTML = compensations.length ? compensations.map((c, i) => {
        ...
        return `
            <div class="chain-item chain-item-block">
                ...
                <p>Fixe : ${formatMontant(c.salaire_fixe)}${c.salaire_variable ? ' · Variable : ' + formatMontant(c.salaire_variable) : ''}${delta}</p>
                ${c.note ? `<p>${escapeHtml(c.note)}</p>` : ''}
            </div>
        `;
    }).join('') : `<p class="empty-hint">Aucune rémunération enregistrée pour le moment.</p>`;
}

function planifierSauvegardeNotes() {
    clearTimeout(saveTimer);
    document.getElementById('saveStatus').innerText = "Frappe en cours…";
...
async function sauvegarderNotes() {
    ...
    try {
        ...
        statusEl.innerText = res.ok ? "Enregistré." : "Erreur d'enregistrement.";
    } catch (err) {
        statusEl.innerText = "Erreur d'enregistrement.";
    }
